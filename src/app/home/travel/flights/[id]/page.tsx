...
export default async function FlightDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <nav className="bg-muted/50 px-4 py-3">
        <div className="container mx-auto">
          <div className="flex items-center space-x-2 text-sm text-muted-foreground">
            ...
            <span>/</span>
            <Link href="/home/travel/flights" className="hover:text-foreground">Flights</Link>
            <span>/</span>
            <span className="text-foreground font-medium">Flight {id}</span>
          </div>
        </div>
      </nav>
      ...
      {/* Flight Detail Placeholder */}
      <section className="py-20 px-4 bg-muted/50">
        <div className="container mx-auto max-w-6xl">
          <Card>
            <CardHeader>
              <CardTitle>Flight Details</CardTitle>
              <CardDescription>Information for flight {id}</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-muted-foreground">
                Flight details for {id} will be displayed here.
              </p>
            </CardContent>
          </Card>
        </div>
