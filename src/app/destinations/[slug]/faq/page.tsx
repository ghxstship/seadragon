...
import { Search, HelpCircle, MessageSquare, Phone, Mail, MapPin, Calendar, DollarSign, Users, Star, ThumbsUp, ExternalLink } from "lucide-react"
...
export default function FAQ({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const categories = [
    ...
    { value: "travel", label: "Getting Here", icon: MapPin },
    { value: "accommodation", label: "Where to Stay", icon: Users },
    { value: "activities", label: "Things to Do", icon: Calendar },
    { value: "food", label: "Food & Dining", icon: DollarSign },
    { value: "safety", label: "Safety & Health", icon: Star },
    { value: "practical", label: "Practical Tips", icon: Phone }
  ]

  const contactInfo: ContactInfo[] = [
    {
      type: "phone",
      label: "Tourist Information Hotline",
      value: "1-800-VISIT-" + destinationName.substring(0, 4).toUpperCase(),
      available: "24/7"
    },
    {
      type: "email",
      label: "Visitor Services",
      value: "info@" + slug.toLowerCase() + "tourism.org",
      available: "Mon-Fri, 9AM-5PM"
    },
    {
      type: "website",
      label: "Official Tourism Site",
      value: "https://www.visit" + slug.toLowerCase() + ".org",
  ...
  useEffect(() => {
    ...
    const loadFaqs = async () => {
      try {
        const res = await fetch(`/api/v1/destinations/${slug}/faq`)
        if (res.ok) {
          const data = await res.json()
          const faqsData = Array.isArray(data.faqs) ? data.faqs : []
          const mapped: FAQItem[] = faqsData.map((f: FAQApiResponse) => ({
            id: String(f.id),
  ...
  const getCategoryIcon = (category: string) => {
    const iconEntry = categories.find(c => c.value === category)
    const Icon = iconEntry?.icon ?? HelpCircle
    return <Icon className="h-5 w-5 text-accent-primary mt-0.5 flex-shrink-0"/>
  }
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-8">
              Find answers to commonly asked questions about visiting {destinationName}.
              Get the information you need for a perfect trip.
            </p>
                ...
                <Accordion type="single" collapsible className="w-full space-y-4">
                  {filteredFaqs.map((faq) => (
                    <AccordionItem key={faq.id} value={faq.id} className="border rounded-lg px-6">
                      <AccordionTrigger value={faq.id} className="hover:no-underline py-6">
